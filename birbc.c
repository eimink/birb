...
static int write_js(const char *filename, birb_song *song) {
    ...
    /* Song data as JS arrays (compresses better than binary string under Brotli) */
    fprintf(f, "// Generated by birbc --js\n");
    ...
    /* order list — flat array, decode in groups of 4 */
    fprintf(f, "O=[");
    for (int i = 0; i < song->order_length; i++) {
        if (i) fprintf(f, ",");
    ...
    /* patterns as a single flat array per plane, indexed by [pat*N*rows + ch*rows + row]
     * This avoids nested array syntax overhead */
    int max_rows = 0;
    for (int p = 0; p < song->num_patterns; p++) {
        int nr = song->pattern_lengths[p];
        if (!nr) nr = 64;
        if (nr > max_rows) max_rows = nr;
    }

    /* Emit pattern lengths */
    fprintf(f, "pl=[");
    for (int p = 0; p < song->num_patterns; p++) {
        if (p) fprintf(f, ",");
    ...
    /* Emit 4 flat arrays: notes, instruments, effects, params
     * Index: pat * N * W + ch * W + row */
    ...
    fprintf(f, "P=(a,p,c,r)=>a[(p*N+c)*W+r],\n");

    /* inline synth engine — render only, ultra-compact */
    fprintf(f,
        "bf=[24,26,27,29,31,32,34,36,38,41,43,46],\n"
        "dv=[8192,16384,32768,49152],\n"
        "nf=n=>(n=n<0?0:n>95?95:n,bf[n%%12]<<(n/12)),\n"
        "spt=S*5/((bpm||125)*2)|0,T=ol*W*tpr*spt,\n"
        "out=new Float32Array(T),ch=[],ct=0,cr=0,op=0,tc=0,c\n"
        "for(c=0;c<N;c++)ch[c]={p:0,f:0,b:0,w:0,n:0,u:F/2,e:0,t:0,a:0,d:0,s:0,r:0,q:0,g:0,x:0,y:0,k:0,l:0,h:0x7FFF,j:16,m:0,i:0,v:255,rv:255,vs:0}\n"
        "function R(){for(c=0;c<N;c++){var q=O[op][c],C=ch[c];if(q>=np)continue\n"
        "var n=P(pn,q,c,cr),ii=P(pi,q,c,cr),rv=P(pv,q,c,cr),fx=P(pf,q,c,cr),pm=P(pp,q,c,cr)\n"
        ...
        "C.q=j[6];C.g=j[7];C.x=j[8];C.y=j[9];C.v=j[10]||255;C.rv=255;C.k=0;C.l=0\n"
        "if(j[0]>=3){C.h=0x7FFF;C.m=0;C.j=256>>(s/12)||1}}}\n"
        "if(rv)C.rv=rv\n"
        "if(fx==1){C.x=pm>>4;C.y=pm&15;C.k=0}\n"
        "else if(fx==2)C.l=pm<<2;else if(fx==3)C.l=-(pm<<2);else if(fx==6){var u=pm>>4,d=pm&15;C.vs=u?u:-d}}}\n"
        "R();\n" /* process row 0 immediately */
        ...
        "if(C.vs){C.rv+=C.vs*4;if(C.rv>255)C.rv=255;if(C.rv<1)C.rv=1}}}\n"
        "for(var i=0;i<T;i++){if(tc<=0){K();tc=spt}tc--\n"
    ...
    /* return: Float32Array + metadata. Demo creates AudioBuffer + playback. */
    fprintf(f,
